...
local function Convert_Base(base, died, newforce, surface)
    ...
    if count ~= 0 and math.random(1 + math.sqrt(count)) == 1 then
        ...
        if died then
            table.insert(global.hiveminds, base.surface.create_entity {
                name = base.name,
    ...
    for k, beacon in ipairs(global.beacons) do
        if beacon.valid then
            ...
            if beacon.energy > 0 then
                ...
                local bases = surface.find_entities_filtered {
                    type = "unit-spawner",
                    area = Get_Bounding_Box(beacon.position, global.NE_Buildings.Settings.Search_Distance),
                    force = enemyForce
                } -- search area of thirty around each ACS for spawners
                local turret = surface.find_entities_filtered {
                    type = "turret",
                    area = Get_Bounding_Box(beacon.position, global.NE_Buildings.Settings.Search_Distance),
                    force = enemyForce
                } -- search area of thirty around each ACS for spawners
                ...
                elseif #turret > 0 then
                    ...
                    for i, enemy in ipairs(surface.find_enemy_units(beacon.position,
                        global.NE_Buildings.Settings.Search_Distance)) do -- search area of ten around each ACS
        ...
        for k, mind in ipairs(global.minds) do -- remove mind control from biters not in area of influence
            ...
            if not mind.valid then -- first make sure the enemy is still valid, if not remove them
                ...
                if surface.find_entities_filtered{
                    ...
                    area = Get_Bounding_Box(mind.position, global.NE_Buildings.Settings.Search_Distance)
                }[1] then -- a AlienControlStation is in range

                    controlled = true
                    break
                end
                if not controlled then
                    mind.force = enemyForce
                end
            end
        end
    end
