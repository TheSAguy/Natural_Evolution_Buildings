...
--- Settup Settings
if not global.NE_Buildings then
    global.NE_Buildings = {}
end
if not global.NE_Buildings.Settings then
    global.NE_Buildings.Settings = {}
end
...
local function On_Built(event)
    ...
    if entity.valid and entity.name == "AlienControlStation_Area" then
        ...
        newAlienControlStation = surface.create_entity({
    ...
    if entity.valid and entity.name == "TerraformingStation_New" then
        ...
        T_Station_Container = surface.create_entity({
        ...
        T_Station_Container.set_request_slot({
            name = "Alien-Stimulant",
            count = "10"
        }, 1)

        global.Terraforming_Station_Table[entity.unit_number] = {
            radar = entity,
            inventory = T_Station_Container
        }
...
local function On_Remove(event)
    ...
    if entity.valid and event.name ~= defines.events.on_entity_died then
        if (entity.name == "small-worm-turret-player" or entity.name == "medium-worm-turret-player" or entity.name ==
            "big-worm-turret-player" or entity.name == "behemoth-worm-turret-player" or entity.name ==
            "Natural_Evolution_Biter-Spawner" or entity.name == "Natural_Evolution_Spitter-Spawner" or entity.name ==
            "ne-living-wall") then
...
local function On_Death(event)
    ...
    --------- Spawner killed
    if entity.valid and (entity.type == "unit-spawner") and (entity.force == game.forces.enemy) and
        global.NE_Buildings.Settings.Battle_Marker then

        -- writeDebug("Enemy Spawner Killed")
        local force = event.force

        Battle_Marker = surface.create_entity({
            name = "battle_marker",
            position = pos,
            force = force
        })
    ...
    if event.force ~= nil and entity.force.name == "enemy" and entity.type == "unit" and event.cause then
        ...
        if ammo and ammo.valid_for_read and AMMO_TYPES[ammo.name] then
            local Convert = surface.create_entity({
                name = entity.name,
                position = pos,
                force = event.cause.force.name
            })
...
Event.register(defines.events.on_sector_scanned, function(event)
    ...
    if event.radar.name == "Thumper" then
        event.radar.surface.set_multi_command {
            command = {
            ...
            for current_index = 1, #global.laid_pheromone_concrete do
                ...
                if currentTilename == "pheromone_concrete" then
                    ...
                    if current_index ~= last_element_index then
                        global.laid_pheromone_concrete[last_element_index] =
                            global.laid_pheromone_concrete[current_index]
            ...
            for i = 1, #global.laid_pheromone_concrete do
                ...
                if currentTilename == "pheromone_concrete" then
                    ...
                    table.insert(New_tiles, {
                        ...
                        position = global.laid_pheromone_concrete[i].position
                    })
...
local function pheromone_concrete_laid(event, surface)
    ...
    for i, vv in ipairs(event.tiles) do
        ...
        if currentTilename == "pheromone_concrete" then
            ...
            table.insert(global.laid_pheromone_concrete, {
                position = vv.position,
                time = event.tick + pheromone_time,
                surface = surface
            })
            table.sort(global.laid_pheromone_concrete, function(a, b)
...
local function Player_Tile_Built(event)
    ...
    if event.tiles then
        pheromone_concrete_laid(event, surface)
    end
