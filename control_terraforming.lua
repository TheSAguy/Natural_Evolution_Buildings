...
function Reduce_Evolution(TS_table)
    ...
    if ammo > 0 and AmmoType == "Alien-Stimulant" then
        ...
        reduction = ((global.deduction_constant * global.factormultiplier) *
                        ((((1 - game.forces.enemy.evolution_factor) ^ 2) * 2) +
                            (game.forces.enemy.evolution_factor * (1 - game.forces.enemy.evolution_factor))) / 3) -- Tweak:  ((1 - evo) ^ 2 * 2 + (Evo * (1 - evo))) / 3 see: http://i.imgur.com/gxJOqco.png 
        ...
        if game.forces.enemy.evolution_factor > 0.95 and game.forces.enemy.evolution_factor > reduction95 then
            ...
            global.Total_TerraformingStations_Evo_Deduction =
                global.Total_TerraformingStations_Evo_Deduction + reduction95
        ...
        elseif game.forces.enemy.evolution_factor > reduction then
            ...
            global.Total_TerraformingStations_Evo_Deduction =
                global.Total_TerraformingStations_Evo_Deduction + reduction
            game.forces.player.evolution_factor = game.forces.enemy.evolution_factor
        ...
        if ammo > 0 then
            TS_table.inventory.get_inventory(1).insert({
                name = AmmoType,
                count = ammo
            })
        end
