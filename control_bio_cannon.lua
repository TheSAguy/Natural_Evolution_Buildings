...
local function Bio_Cannon_Check(Bio_Cannon_List)
    -- ~ NEBuildings.writeDebug("Entered function Bio_Cannon_List")
    ...
    if ammo > 0 and Bio_Cannon_List[2].energy > 0 then
        ...
        -- NEBuildings.writeDebug("The Number of Spawners are: " .. tostring(#spawner))
        -- NEBuildings.writeDebug("The Number of Worms are: " .. tostring(#worms))
        -- Find Spawner Target
        if #spawner > 0 and target == nil then
            for _, enemy in pairs(spawner) do
                local distance = math.sqrt(((Bio_Cannon.position.x - enemy.position.x) ^ 2) +
                                               ((Bio_Cannon.position.y - enemy.position.y) ^ 2))
                -- NEBuildings.writeDebug("The Distance is: " .. tostring(distance))
                if (distance > 20) and (distance < (radius + 1)) and target == nil then
                    -- ~ if target == nil then
                    target = {enemy}
    ...
    -- ~ NEBuildings.writeDebug("End of function Bio_Cannon_List")
end
        ...
        if global.Bio_Cannon_Counter == 0 or global.Bio_Cannon_Counter == nil then
            ...
            for ix, vx in pairs(global.Bio_Cannon_Table) do
                if vx[1].valid and vx[2].valid then
                    ...
                    global.Bio_Cannon_Table[ix] = nil
                end
            end
        else
