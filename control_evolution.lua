...
local function UpdateUnitsCommands(player_index)
    ...
    local aggression_area = {{pos.x - agro_area_rad, pos.y - agro_area_rad},
                             {pos.x + agro_area_rad, pos.y + agro_area_rad}}
    ...
    if closest_index == -1 then
        local call_back_area = {{pos.x - call_back_area_rad, pos.y - call_back_area_rad},
                                {pos.x + call_back_area_rad, pos.y + call_back_area_rad}}
        ...
        for index, biter in ipairs(biters) do
            if biter.force == (player.force) then
                biter.set_command({
                    ...
                    destination = pos,
                    radius = 10,
                    distraction = defines.distraction.byanything
                });
